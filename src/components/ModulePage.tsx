  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-[#080705] text-pearl">
      ...
      <section className="relative min-h-[88svh] overflow-hidden">
        ...
        <div className="relative mx-auto flex min-h-[88svh] max-w-[96rem] flex-col justify-end px-5 pb-14 pt-32 sm:px-8 lg:px-12 lg:pb-20">
          <p className="mb-7 text-[0.62rem] uppercase tracking-[0.5em] text-[#d3b98d]">
            {module.eyebrow}
          </p>
          <h1 className="max-w-6xl font-serif text-[4.5rem] leading-[0.82] text-pearl sm:text-[8rem] lg:text-[11rem]">
            {module.title}
          </h1>
          <p className="mt-9 max-w-3xl text-lg leading-8 text-[#d8d0c2] sm:text-2xl sm:leading-10">
            {module.intro}
          </p>
        </div>
      </section>
        ...
        <div className="grid gap-px overflow-hidden border border-white/10 bg-white/10 sm:grid-cols-2">
          {module.focus.map((item) => (
            <div key={item} className="bg-[#100d09] p-6 sm:p-8">
              ...
              <p className="mt-4 font-serif text-3xl leading-none">{item}</p>
            </div>
          ))}
        </div>
      </section>

      <section className="mx-auto max-w-[96rem] px-5 py-8 sm:px-8 lg:px-12 lg:py-16">
        <div className="grid gap-4 md:grid-cols-3">
          {module.images.map((image) => (
            <div key={image} className="relative min-h-[24rem] overflow-hidden bg-black">
